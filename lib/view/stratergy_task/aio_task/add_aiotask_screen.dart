...
import 'aio_stratergy_screen.dart';


class AddAEOTaskScreen extends StatefulWidget {
  const AddAEOTaskScreen({super.key});

  @override
  State<AddAEOTaskScreen> createState() => _AddAEOTaskScreenState();
}

class _AddAEOTaskScreenState extends State<AddAEOTaskScreen> {
  ...
  gettaskdb() async {
    aiogettask = await DatabaseHelper().getAIOdata();
    print(">>>>>>>>>>>>getseoTask>>>>>>>${aiogettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: aiogettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertAIOtaskdata(addTask);
      Get.to(AioStrategyScreen(businessId: Helper.businessId,));
