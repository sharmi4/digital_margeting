...
import 'comm_m_statergy_screen.dart';


class AddCommunityMTaskScreen extends StatefulWidget {
  const AddCommunityMTaskScreen({super.key});

  @override
  State<AddCommunityMTaskScreen> createState() => _AddCommunityMTaskScreenState();
}

class _AddCommunityMTaskScreenState extends State<AddCommunityMTaskScreen> {
  ...
  gettaskdb() async {
    commumgettask = await DatabaseHelper().getCOMMUMdata();
    print(">>>>>>>>>>>>getcmTask>>>>>>>${commumgettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: commumgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertCOMMUMtaskdata(addTask);
      Get.to(CommunityMStrategyScreen(businessId: Helper.businessId,));
