...
import 'add_comm_m_screen.dart';
import 'comm_m_particulartask_screen.dart';


class CommunityMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  CommunityMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<CommunityMStrategyScreen> createState() => _CommunityMStrategyScreenState();
}

class _CommunityMStrategyScreenState extends State<CommunityMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getcommumTask>>>>>>>${commumgettask.length}");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          Column(
            ...
            children: [
              ...
              ListView.builder(
                ...
                shrinkWrap: true,
                itemCount: commumgettask.length,
      ...
      floatingActionButton: FloatingActionButton(
        ...
        elevation: 1,
        onPressed: (){
          Get.to(AddCommunityMTaskScreen());
        },
  ...
  Widget _buildProgressCard() {
    int completed = commumgettask.where((task) => task["iscompleted"] == "1").length;
              ...
              RichText(
                text: TextSpan(
                  ...
                  children: [
                    ...
                    TextSpan(
  text: " / ${commumgettask.length} Tasks",
  style: primaryFont(
  ...
  Widget _buildTaskTile(int index) {
    final task = commumgettask[index];
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.commumselectedstartergytitle = commumgettask[index]["taskname"];
                Helper.commumtaskId = commumgettask[index]["id"].toString();
                      Helper.commumsnumber = commumgettask[index]["snumber"];
                      Helper.COMMUMinstrusction = commumgettask[index]["insturctions"];
                      Helper.commumtype = commumgettask[index]["type"];

                      Get.to(
                        CommunityMParticulartaskProgressScreen(
                          instructions: Helper.COMMUMinstrusction, 
                          category: 'COMMUNITY M', type: Helper.commumtype,
                        ),
                      );
              },
              child: Text(
                commumgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(commumgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.commumtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateCOMMUMtask(updatedTask);
