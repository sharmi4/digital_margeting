...
import 'add_imtask_screen.dart';
import 'im_particulartask_screen.dart';





class IMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  IMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<IMStrategyScreen> createState() => _IMStrategyScreenState();
}

class _IMStrategyScreenState extends State<IMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getemTask>>>>>>>${imgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = imgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.imselectedstartergytitle = imgettask[index]["taskname"];
                Helper.imtaskId = imgettask[index]["id"].toString();
                      Helper.imsnumber = imgettask[index]["snumber"];
                      Helper.IMinstrusction = imgettask[index]["insturctions"];
                      Helper.imtype = imgettask[index]["type"];

                      Get.to(
                        IMParticulartaskProgressScreen(
                          instructions: Helper.IMinstrusction, 
                          category: 'IM', type: Helper.imtype,
                        ),
                      );
              },
              child: Text(
                imgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(imgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.imtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateIMtask(updatedTask);
