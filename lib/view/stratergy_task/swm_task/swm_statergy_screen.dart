...
import 'add_swmtask_screen.dart';
import 'swm_particulartask_screen.dart';


class SWMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  SWMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<SWMStrategyScreen> createState() => _SWMStrategyScreenState();
}

class _SWMStrategyScreenState extends State<SWMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getswmTask>>>>>>>${swmgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = swmgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.swmselectedstartergytitle = swmgettask[index]["taskname"];
                Helper.swmtaskId = swmgettask[index]["id"].toString();
                      Helper.swmsnumber = swmgettask[index]["snumber"];
                      Helper.SWMinstrusction = swmgettask[index]["insturctions"];
                      Helper.swmtype = swmgettask[index]["type"];

                      Get.to(
                        SWMParticulartaskProgressScreen(
                          instructions: Helper.SWMinstrusction, 
                          category: 'SWM', type: Helper.swmtype,
                        ),
                      );
              },
              child: Text(
                swmgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(swmgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.swmtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateSWMtask(updatedTask);
