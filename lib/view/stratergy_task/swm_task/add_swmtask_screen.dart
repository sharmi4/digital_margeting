...
import 'swm_statergy_screen.dart';


class AddSWMTaskScreen extends StatefulWidget {
  const AddSWMTaskScreen({super.key});

  @override
  State<AddSWMTaskScreen> createState() => _AddSWMTaskScreenState();
}

class _AddSWMTaskScreenState extends State<AddSWMTaskScreen> {
  ...
  gettaskdb() async {
    swmgettask = await DatabaseHelper().getSWMdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: swmgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertSWMtaskdata(addTask);
      Get.to(SWMStrategyScreen(businessId: Helper.businessId,));
