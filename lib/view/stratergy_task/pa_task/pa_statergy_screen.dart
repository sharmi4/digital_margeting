...
import 'add_patask_screen.dart';
import 'pa_particulartask_screen.dart';





class PAStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  PAStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<PAStrategyScreen> createState() => _PAStrategyScreenState();
}

class _PAStrategyScreenState extends State<PAStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getpaTask>>>>>>>${pagettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = pagettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.paselectedstartergytitle = pagettask[index]["taskname"];
                Helper.pataskId = pagettask[index]["id"].toString();
                      Helper.pasnumber = pagettask[index]["snumber"];
                      Helper.PAinstrusction = pagettask[index]["insturctions"];
                      Helper.patype = pagettask[index]["type"];

                      Get.to(
                        PAParticulartaskProgressScreen(
                          instructions: Helper.PAinstrusction, 
                          category: 'PA', type: Helper.patype,
                        ),
                      );
              },
              child: Text(
                pagettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(pagettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.patype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updatePAtask(updatedTask);
