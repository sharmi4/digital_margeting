...
import 'pa_statergy_screen.dart';





class AddPATaskScreen extends StatefulWidget {
  const AddPATaskScreen({super.key});

  @override
  State<AddPATaskScreen> createState() => _AddPATaskScreenState();
}

class _AddPATaskScreenState extends State<AddPATaskScreen> {
  ...
  gettaskdb() async {
    pagettask = await DatabaseHelper().getPAdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: pagettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertPAtaskdata(addTask);
      Get.to(PAStrategyScreen(businessId: Helper.businessId,));
