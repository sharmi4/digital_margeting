...
import 'ppc_statergy_screen.dart';




class AddPPCTaskScreen extends StatefulWidget {
  const AddPPCTaskScreen({super.key});

  @override
  State<AddPPCTaskScreen> createState() => _AddPPCTaskScreenState();
}

class _AddPPCTaskScreenState extends State<AddPPCTaskScreen> {
  ...
  gettaskdb() async {
    ppcgettask = await DatabaseHelper().getPPCdata();
    print(">>>>>>>>>>>>getwatTask>>>>>>>${ppcgettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: ppcgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertPPCtaskdata(addTask);
      Get.to(PPCStrategyScreen(businessId: Helper.businessId,));
