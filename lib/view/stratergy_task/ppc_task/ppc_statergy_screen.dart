...
import 'add_ppctask_screen.dart';
import 'ppc_particulartask_screen.dart';




class PPCStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  PPCStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<PPCStrategyScreen> createState() => _PPCStrategyScreenState();
}

class _PPCStrategyScreenState extends State<PPCStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getppcTask>>>>>>>${ppcgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = ppcgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.ppcselectedstartergytitle = ppcgettask[index]["taskname"];
                Helper.ppctaskId = ppcgettask[index]["id"].toString();
                      Helper.ppcsnumber = ppcgettask[index]["snumber"];
                      Helper.PPCinstrusction = ppcgettask[index]["insturctions"];
                      Helper.ppctype = ppcgettask[index]["type"];

                      Get.to(
                        PPCParticulartaskProgressScreen(
                          instructions: Helper.PPCinstrusction, 
                          category: 'PPC', type: Helper.ppctype,
                        ),
                      );
              },
              child: Text(
                ppcgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(ppcgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.ppctype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updatePPCtask(updatedTask);
