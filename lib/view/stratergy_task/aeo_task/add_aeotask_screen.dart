...
class _AddAEOTaskScreenState extends State<AddAEOTaskScreen> {
  ...
  gettaskdb() async {
    aeogettask = await DatabaseHelper().getAEOdata();
    print(">>>>>>>>>>>>getseoTask>>>>>>>${aeogettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: aeogettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertAEOtaskdata(addTask);
      Get.to(AeoStrategyScreen(businessId: Helper.businessId,));
