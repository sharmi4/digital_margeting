...
import 'add_dptask_screen.dart';
import 'dp_particulartask_screen.dart';





class DPStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  DPStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<DPStrategyScreen> createState() => _DPStrategyScreenState();
}

class _DPStrategyScreenState extends State<DPStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getdpTask>>>>>>>${dpgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = dpgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.dpselectedstartergytitle = dpgettask[index]["taskname"];
                Helper.dptaskId = dpgettask[index]["id"].toString();
                      Helper.dpsnumber = dpgettask[index]["snumber"];
                      Helper.DPinstrusction = dpgettask[index]["insturctions"];
                      Helper.dptype = dpgettask[index]["type"];

                      Get.to(
                        DPParticulartaskProgressScreen(
                          instructions: Helper.DPinstrusction, 
                          category: 'DP', type: Helper.dptype,
                        ),
                      );
              },
              child: Text(
                dpgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(dpgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.dptype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateDPtask(updatedTask);
