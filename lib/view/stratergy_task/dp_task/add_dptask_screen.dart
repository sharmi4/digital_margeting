...
import 'dp_statergy_screen.dart';


class AddDPTaskScreen extends StatefulWidget {
  const AddDPTaskScreen({super.key});

  @override
  State<AddDPTaskScreen> createState() => _AddDPTaskScreenState();
}

class _AddDPTaskScreenState extends State<AddDPTaskScreen> {
  ...
  gettaskdb() async {
    dpgettask = await DatabaseHelper().getDPdata();
    print(">>>>>>>>>>>>getcmTask>>>>>>>${dpgettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: dpgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertDPtaskdata(addTask);
      Get.to(DPStrategyScreen(businessId: Helper.businessId,));
