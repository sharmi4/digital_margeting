...
import 'vso_statergy_screen.dart';



class AddVSOTaskScreen extends StatefulWidget {
  const AddVSOTaskScreen({super.key});

  @override
  State<AddVSOTaskScreen> createState() => _AddVSOTaskScreenState();
}

class _AddVSOTaskScreenState extends State<AddVSOTaskScreen> {
  ...
  gettaskdb() async {
    vsogettask = await DatabaseHelper().getVSOdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: vsogettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertVSOtaskdata(addTask);
      Get.to(VSOStrategyScreen(businessId: Helper.businessId,));
