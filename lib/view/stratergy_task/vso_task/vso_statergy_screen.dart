...
import 'add_vsotask_screen.dart';
import 'vso_particulartask_screen.dart';




class VSOStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  VSOStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<VSOStrategyScreen> createState() => _VSOStrategyScreenState();
}

class _VSOStrategyScreenState extends State<VSOStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getvsoTask>>>>>>>${vsogettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = vsogettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.vsoselectedstartergytitle = vsogettask[index]["taskname"];
                Helper.vsotaskId = vsogettask[index]["id"].toString();
                      Helper.vsosnumber = vsogettask[index]["snumber"];
                      Helper.VSOinstrusction = vsogettask[index]["insturctions"];
                      Helper.vsotype = vsogettask[index]["type"];

                      Get.to(
                        VSOParticulartaskProgressScreen(
                          instructions: Helper.VSOinstrusction, 
                          category: 'VSO', type: Helper.vsotype,
                        ),
                      );
              },
              child: Text(
                vsogettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(vsogettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.vsotype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateVSOtask(updatedTask);
