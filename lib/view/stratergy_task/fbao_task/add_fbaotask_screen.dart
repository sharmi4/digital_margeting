...
import 'fbao_statergy_screen.dart';



class AddFBAOTaskScreen extends StatefulWidget {
  const AddFBAOTaskScreen({super.key});

  @override
  State<AddFBAOTaskScreen> createState() => _AddFBAOTaskScreenState();
}

class _AddFBAOTaskScreenState extends State<AddFBAOTaskScreen> {
  ...
  gettaskdb() async {
    fbaogettask = await DatabaseHelper().getFBAOdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: fbaogettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertFBAOtaskdata(addTask);
      Get.to(FBAOStrategyScreen(businessId: Helper.businessId,));
