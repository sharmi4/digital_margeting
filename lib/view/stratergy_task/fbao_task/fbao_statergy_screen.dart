...
import 'add_fbaotask_screen.dart';
import 'fbao_particulartask_screen.dart';



class FBAOStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  FBAOStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<FBAOStrategyScreen> createState() => _FBAOStrategyScreenState();
}

class _FBAOStrategyScreenState extends State<FBAOStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getemTask>>>>>>>${fbaogettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = fbaogettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.fbaoselectedstartergytitle = fbaogettask[index]["taskname"];
                Helper.fbaotaskId = fbaogettask[index]["id"].toString();
                      Helper.fbaosnumber = fbaogettask[index]["snumber"];
                      Helper.FBAOinstrusction = fbaogettask[index]["insturctions"];
                      Helper.fbaotype = fbaogettask[index]["type"];

                      Get.to(
                        FBAOParticulartaskProgressScreen(
                          instructions: Helper.FBAOinstrusction, 
                          category: 'FBAO', type: Helper.fbaotype,
                        ),
                      );
              },
              child: Text(
                fbaogettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(fbaogettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.fbaotype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateFBAOtask(updatedTask);
