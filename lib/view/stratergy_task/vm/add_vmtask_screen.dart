...
import 'vm_statergy_screen.dart';

class AddVMTaskScreen extends StatefulWidget {
  const AddVMTaskScreen({super.key});

  @override
  State<AddVMTaskScreen> createState() => _AddVMTaskScreenState();
}

class _AddVMTaskScreenState extends State<AddVMTaskScreen> {
  ...
  gettaskdb() async {
    vmgettask = await DatabaseHelper().getVMdata();
    print(">>>>>>>>>>>>getvmTask>>>>>>>${vmgettask.length}");
    setState(() {});
  }
      ...
      bottomNavigationBar: Padding(
        ...
        child: InkWell(
          onTap: () async {
            ...
            await DatabaseHelper().insertVMtaskdata(addTask);
            Get.to(VMStrategyScreen(businessId: Helper.businessId));
          },
          child: Container(
            height: 50,
