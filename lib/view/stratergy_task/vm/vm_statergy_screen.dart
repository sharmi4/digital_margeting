...
import 'add_vmtask_screen.dart';
import 'vm_particulartask_screen.dart';



class VMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  VMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<VMStrategyScreen> createState() => _VMStrategyScreenState();
}

class _VMStrategyScreenState extends State<VMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getemTask>>>>>>>${vmgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = vmgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.vmselectedstartergytitle = vmgettask[index]["taskname"];
                Helper.vmtaskId = vmgettask[index]["id"].toString();
                      Helper.vmsnumber = vmgettask[index]["snumber"];
                      Helper.VMinstrusction = vmgettask[index]["insturctions"];
                      Helper.vmtype = vmgettask[index]["type"];

                      Get.to(
                        VMParticulartaskProgressScreen(
                          instructions: Helper.VMinstrusction, 
                          category: 'VM', type: Helper.vmtype,
                        ),
                      );
              },
              child: Text(
                vmgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(vmgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.vmtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateVMtask(updatedTask);
