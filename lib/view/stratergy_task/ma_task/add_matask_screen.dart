...
import 'ma_statergy_screen.dart';


class AddMATaskScreen extends StatefulWidget {
  const AddMATaskScreen({super.key});

  @override
  State<AddMATaskScreen> createState() => _AddMATaskScreenState();
}

class _AddMATaskScreenState extends State<AddMATaskScreen> {
  ...
  gettaskdb() async {
    magettask = await DatabaseHelper().getMAdata();
    print(">>>>>>>>>>>>getMATask>>>>>>>${magettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: magettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertMAtaskdata(addTask);
      Get.to(MAStrategyScreen(businessId: Helper.businessId,));
