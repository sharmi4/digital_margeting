...
import 'add_matask_screen.dart';
import 'ma_particulartask_screen.dart';




class MAStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  MAStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<MAStrategyScreen> createState() => _MAStrategyScreenState();
}

class _MAStrategyScreenState extends State<MAStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getlsTask>>>>>>>${magettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = magettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.maselectedstartergytitle = magettask[index]["taskname"];
                Helper.mataskId = magettask[index]["id"].toString();
                      Helper.masnumber = magettask[index]["snumber"];
                      Helper.MAinstrusction = magettask[index]["insturctions"];
                      Helper.matype = magettask[index]["type"];

                      Get.to(
                        MAParticulartaskProgressScreen(
                          instructions: Helper.MAinstrusction, 
                          category: 'MA', type: Helper.matype,
                        ),
                      );
              },
              child: Text(
                magettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(magettask[index]);
  ...
  builder: (context) {
    ...
    await DatabaseHelper().updateMAtask(updatedTask);
