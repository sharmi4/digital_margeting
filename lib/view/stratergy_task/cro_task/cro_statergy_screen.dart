...
import 'add_crotask_screen.dart';
import 'cro_particulartask_screen.dart';



class CROStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  CROStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<CROStrategyScreen> createState() => _CROStrategyScreenState();
}

class _CROStrategyScreenState extends State<CROStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getcroTask>>>>>>>${crogettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = crogettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.croselectedstartergytitle = crogettask[index]["taskname"];
                Helper.crotaskId = crogettask[index]["id"].toString();
                      Helper.crosnumber = crogettask[index]["snumber"];
                      Helper.CROinstrusction = crogettask[index]["insturctions"];
                      Helper.crotype = crogettask[index]["type"];

                      Get.to(
                        CROParticulartaskProgressScreen(
                          instructions: Helper.CROinstrusction, 
                          category: 'CRO', type: Helper.crotype,
                        ),
                      );
              },
              child: Text(
                crogettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(crogettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.crotype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateCROtask(updatedTask);
