...
import 'add_geotask_screen.dart';
import 'geo_particulartask_screen.dart';



class GeoStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  GeoStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<GeoStrategyScreen> createState() => _GeoStrategyScreenState();
}

class _GeoStrategyScreenState extends State<GeoStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getgeoTask>>>>>>>${geogettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = geogettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.geoselectedstartergytitle = geogettask[index]["taskname"];
                Helper.geotaskId = geogettask[index]["id"].toString();
                      Helper.geosnumber = geogettask[index]["snumber"];
                      Helper.GEOinstrusction = geogettask[index]["insturctions"];
                      Helper.geotype = geogettask[index]["type"];
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(geogettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.geotype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateGEOtask(updatedTask);
