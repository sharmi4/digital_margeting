...
import 'cm_statergy_screen.dart';


class AddCMTaskScreen extends StatefulWidget {
  const AddCMTaskScreen({super.key});

  @override
  State<AddCMTaskScreen> createState() => _AddCMTaskScreenState();
}

class _AddCMTaskScreenState extends State<AddCMTaskScreen> {
  ...
  gettaskdb() async {
    cmgettask = await DatabaseHelper().getCMdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: cmgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertCMtaskdata(addTask);
      Get.to(CMStrategyScreen(businessId: Helper.businessId,));
