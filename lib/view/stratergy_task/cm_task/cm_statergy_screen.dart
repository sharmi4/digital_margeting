...
import 'add_cmtask_screen.dart';
import 'cm_particulartask_screen.dart';




class CMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  CMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<CMStrategyScreen> createState() => _CMStrategyScreenState();
}

class _CMStrategyScreenState extends State<CMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getcmTask>>>>>>>${cmgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = cmgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.cmselectedstartergytitle = cmgettask[index]["taskname"];
                Helper.cmtaskId = cmgettask[index]["id"].toString();
                      Helper.cmsnumber = cmgettask[index]["snumber"];
                      Helper.CMinstrusction = cmgettask[index]["insturctions"];
                      Helper.cmtype = cmgettask[index]["type"];

                      Get.to(
                        CMParticulartaskProgressScreen(
                          instructions: Helper.CMinstrusction, 
                          category: 'CM', type: Helper.cmtype,
                        ),
                      );
              },
              child: Text(
                cmgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(cmgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.cmtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateCMtask(updatedTask);
