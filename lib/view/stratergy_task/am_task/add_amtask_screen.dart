...
import 'am_statergy_screen.dart';



class AddAMTaskScreen extends StatefulWidget {
  const AddAMTaskScreen({super.key});

  @override
  State<AddAMTaskScreen> createState() => _AddAMTaskScreenState();
}

class _AddAMTaskScreenState extends State<AddAMTaskScreen> {
  ...
  gettaskdb() async {
    amgettask = await DatabaseHelper().getAMdata();
    print(">>>>>>>>>>>>getamTask>>>>>>>${amgettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: amgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertAMtaskdata(addTask);
      Get.to(AMStrategyScreen(businessId: Helper.businessId,));
