...
import 'add_amtask_screen.dart';
import 'am_particulartask_screen.dart';
...
class AMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  AMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<AMStrategyScreen> createState() => _AMStrategyScreenState();
}

class _AMStrategyScreenState extends State<AMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getemTask>>>>>>>${amgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = amgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.amselectedstartergytitle = amgettask[index]["taskname"];
                Helper.amtaskId = amgettask[index]["id"].toString();
                      Helper.amsnumber = amgettask[index]["snumber"];
                      Helper.AMinstrusction = amgettask[index]["insturctions"];
                      Helper.amtype = amgettask[index]["type"];

                      Get.to(
                        AMParticulartaskProgressScreen(
                          instructions: Helper.AMinstrusction, 
                          category: 'AM', type: Helper.amtype,
                        ),
                      );
              },
              child: Text(
                amgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(amgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.amtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateAMtask(updatedTask);
