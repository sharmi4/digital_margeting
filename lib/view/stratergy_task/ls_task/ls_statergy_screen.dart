...
import 'add_lstask_screen.dart';
import 'ls_particulartask_screen.dart';




class LSStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  LSStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<LSStrategyScreen> createState() => _LSStrategyScreenState();
}

class _LSStrategyScreenState extends State<LSStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getlsTask>>>>>>>${lsgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = lsgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.lsselectedstartergytitle = lsgettask[index]["taskname"];
                Helper.lstaskId = lsgettask[index]["id"].toString();
                      Helper.lssnumber = lsgettask[index]["snumber"];
                      Helper.LSinstrusction = lsgettask[index]["insturctions"];
                      Helper.lstype = lsgettask[index]["type"];

                      Get.to(
                        LSParticulartaskProgressScreen(
                          instructions: Helper.LSinstrusction, 
                          category: 'LS', type: Helper.lstype,
                        ),
                      );
              },
              child: Text(
                lsgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(lsgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.lstype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateLStask(updatedTask);
