...
import 'ls_statergy_screen.dart';


class AddLSTaskScreen extends StatefulWidget {
  const AddLSTaskScreen({super.key});

  @override
  State<AddLSTaskScreen> createState() => _AddLSTaskScreenState();
}

class _AddLSTaskScreenState extends State<AddLSTaskScreen> {
  ...
  gettaskdb() async {
    lsgettask = await DatabaseHelper().getLSdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: lsgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertLStaskdata(addTask);
      Get.to(LSStrategyScreen(businessId: Helper.businessId,));
