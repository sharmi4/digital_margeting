...
import 'add_wvemtask_screen.dart';
import 'wvem_particulartask_screen.dart';




class WVEMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  WVEMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<WVEMStrategyScreen> createState() => _WVEMStrategyScreenState();
}

class _WVEMStrategyScreenState extends State<WVEMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getwatTask>>>>>>>${wvemgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = wvemgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.wvemselectedstartergytitle = wvemgettask[index]["taskname"];
                Helper.wvemtaskId = wvemgettask[index]["id"].toString();
                      Helper.wvemsnumber = wvemgettask[index]["snumber"];
                      Helper.WVEMinstrusction = wvemgettask[index]["insturctions"];
                      Helper.wvemtype = wvemgettask[index]["type"];

                      Get.to(
                        WVEMParticulartaskProgressScreen(
                          instructions: Helper.WVEMinstrusction, 
                          category: 'WVEM', type: Helper.wvemtype,
                        ),
                      );
              },
              child: Text(
                wvemgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(wvemgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.wvemtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateWVEMtask(updatedTask);
