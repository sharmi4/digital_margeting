...
import 'wvem_statergy_screen.dart';



class AddWVEMTaskScreen extends StatefulWidget {
  const AddWVEMTaskScreen({super.key});

  @override
  State<AddWVEMTaskScreen> createState() => _AddWVEMTaskScreenState();
}

class _AddWVEMTaskScreenState extends State<AddWVEMTaskScreen> {
  ...
  gettaskdb() async {
    wvemgettask = await DatabaseHelper().getWVEMdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: wvemgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertWVEMtaskdata(addTask);
      Get.to(WVEMStrategyScreen(businessId: Helper.businessId,));
