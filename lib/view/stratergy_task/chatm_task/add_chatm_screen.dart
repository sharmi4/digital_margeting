...
import 'chatm_statergy_screen.dart';



class AddCHATMTaskScreen extends StatefulWidget {
  const AddCHATMTaskScreen({super.key});

  @override
  State<AddCHATMTaskScreen> createState() => _AddCHATMTaskScreenState();
}

class _AddCHATMTaskScreenState extends State<AddCHATMTaskScreen> {
  ...
  gettaskdb() async {
    chatmgettask = await DatabaseHelper().getCHATMdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: chatmgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertCHATMtaskdata(addTask);
      Get.to(CHATMStrategyScreen(businessId: Helper.businessId,));
