...
import 'add_chatm_screen.dart';
import 'chatm_particulartask_screen.dart';


class CHATMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  CHATMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<CHATMStrategyScreen> createState() => _CHATMStrategyScreenState();
}

class _CHATMStrategyScreenState extends State<CHATMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getchatmTask>>>>>>>${chatmgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = chatmgettask.where((task) => task["iscompleted"] == "1").length;
              ...
              RichText(
                text: TextSpan(
                  ...
                  children: [
                    ...
                    TextSpan(
  text: " / ${chatmgettask.length} Tasks",
  style: primaryFont(
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.chatmselectedstartergytitle = chatmgettask[index]["taskname"];
                Helper.chatmtaskId = chatmgettask[index]["id"].toString();
                      Helper.chatmsnumber = chatmgettask[index]["snumber"];
                      Helper.CHATMinstrusction = chatmgettask[index]["insturctions"];
                      Helper.chatmtype = chatmgettask[index]["type"];

                      Get.to(
                        CHATMParticulartaskProgressScreen(
                          instructions: Helper.CHATMinstrusction, 
                          category: 'CHATM', type: Helper.chatmtype,
                        ),
                      );
              },
              child: Text(
                chatmgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(chatmgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.chatmtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateCHATMtask(updatedTask);
