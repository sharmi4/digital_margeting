...
import 'lpo_statergy_screen.dart';



class AddLPOTaskScreen extends StatefulWidget {
  const AddLPOTaskScreen({super.key});

  @override
  State<AddLPOTaskScreen> createState() => _AddLPOTaskScreenState();
}

class _AddLPOTaskScreenState extends State<AddLPOTaskScreen> {
  ...
  gettaskdb() async {
    lpogettask = await DatabaseHelper().getLPOdata();
    print(">>>>>>>>>>>>getemTask>>>>>>>${lpogettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: lpogettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertLPOtaskdata(addTask);
      Get.to(LPOStrategyScreen(businessId: Helper.businessId,));
