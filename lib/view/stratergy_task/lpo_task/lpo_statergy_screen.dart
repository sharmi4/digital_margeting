...
import 'add_lpotask_screen.dart';
import 'lpo_particulartask_screen.dart';



class LPOStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  LPOStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<LPOStrategyScreen> createState() => _LPOStrategyScreenState();
}

class _LPOStrategyScreenState extends State<LPOStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getLPOTask>>>>>>>${lpogettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = lpogettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.lposelectedstartergytitle = lpogettask[index]["taskname"];
                Helper.lpotaskId = lpogettask[index]["id"].toString();
                      Helper.lposnumber = lpogettask[index]["snumber"];
                      Helper.LPOinstrusction = lpogettask[index]["insturctions"];
                      Helper.lpotype = lpogettask[index]["type"];

                      Get.to(
                        LPOParticulartaskProgressScreen(
                          instructions: Helper.LPOinstrusction, 
                          category: 'LPO', type: Helper.lpotype,
                        ),
                      );
              },
              child: Text(
                lpogettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(lpogettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.lpotype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateLPOtask(updatedTask);
