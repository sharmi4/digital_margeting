...
import 'sxo_stratergy_screen.dart';


class AddSXOTaskScreen extends StatefulWidget {
  const AddSXOTaskScreen({super.key});

  @override
  State<AddSXOTaskScreen> createState() => _AddSXOTaskScreenState();
}

class _AddSXOTaskScreenState extends State<AddSXOTaskScreen> {
  ...
  gettaskdb() async {
    sxogettask = await DatabaseHelper().getSXOdata();
    print(">>>>>>>>>>>>getseoTask>>>>>>>${sxogettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: sxogettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertSXOtaskdata(addTask);
      Get.to(SxoStrategyScreen(businessId: Helper.businessId,));
