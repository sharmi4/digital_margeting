...
import 'sxo_partuculartask_screen.dart';



class SxoStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  SxoStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<SxoStrategyScreen> createState() => _SxoStrategyScreenState();
}

class _SxoStrategyScreenState extends State<SxoStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getaeoTask>>>>>>>${sxogettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = sxogettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.sxoselectedstartergytitle = sxogettask[index]["taskname"];
                Helper.sxotaskId = sxogettask[index]["id"].toString();
                      Helper.sxosnumber = sxogettask[index]["snumber"];
                      Helper.SXOinstrusction = sxogettask[index]["insturctions"];
                      Helper.sxotype = sxogettask[index]["type"];

                      Get.to(
                        SXOParticulartaskProgressScreen(
                          instructions: Helper.SXOinstrusction, 
                          category: 'SXO', type: Helper.sxotype,
                        ),
                      );
              },
              child: Text(
                sxogettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(sxogettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.aiotype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateSXOtask(updatedTask);
