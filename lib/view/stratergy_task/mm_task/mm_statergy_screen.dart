...
import 'add_mmtask_screen.dart';
import 'mm_particulartask_screen.dart';





class MMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  MMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<MMStrategyScreen> createState() => _MMStrategyScreenState();
}

class _MMStrategyScreenState extends State<MMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getmmTask>>>>>>>${mmgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = mmgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.mmselectedstartergytitle = mmgettask[index]["taskname"];
                Helper.mmtaskId = mmgettask[index]["id"].toString();
                      Helper.mmsnumber = mmgettask[index]["snumber"];
                      Helper.MMinstrusction = mmgettask[index]["insturctions"];
                      Helper.mmtype = mmgettask[index]["type"];

                      Get.to(
                        MMParticulartaskProgressScreen(
                          instructions: Helper.MMinstrusction, 
                          category: 'MM', type: Helper.mmtype,
                        ),
                      );
              },
              child: Text(
                mmgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(mmgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.mmtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateMMtask(updatedTask);
