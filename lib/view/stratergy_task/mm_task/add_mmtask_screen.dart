...
import 'mm_statergy_screen.dart';



class AddMMTaskScreen extends StatefulWidget {
  const AddMMTaskScreen({super.key});

  @override
  State<AddMMTaskScreen> createState() => _AddMMTaskScreenState();
}

class _AddMMTaskScreenState extends State<AddMMTaskScreen> {
  ...
  gettaskdb() async {
    mmgettask = await DatabaseHelper().getMMdata();
    print(">>>>>>>>>>>>getemTask>>>>>>>${mmgettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: mmgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertIMtaskdata(addTask);
      Get.to(MMStrategyScreen(businessId: Helper.businessId,));
