...
import 'wat_statergy_screen.dart';



class AddWATTaskScreen extends StatefulWidget {
  const AddWATTaskScreen({super.key});

  @override
  State<AddWATTaskScreen> createState() => _AddWATTaskScreenState();
}

class _AddWATTaskScreenState extends State<AddWATTaskScreen> {
  ...
  gettaskdb() async {
    watgettask = await DatabaseHelper().getWATdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: watgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertWATtaskdata(addTask);
      Get.to(WATStrategyScreen(businessId: Helper.businessId,));
