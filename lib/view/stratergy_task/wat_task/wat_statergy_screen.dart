...
import 'add_wattask_screen.dart';
import 'wat_particulartask_screen.dart';



class WATStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  WATStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<WATStrategyScreen> createState() => _WATStrategyScreenState();
}

class _WATStrategyScreenState extends State<WATStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getwatTask>>>>>>>${watgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = watgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.watselectedstartergytitle = watgettask[index]["taskname"];
                Helper.wattaskId = watgettask[index]["id"].toString();
                      Helper.watsnumber = watgettask[index]["snumber"];
                      Helper.WATinstrusction = watgettask[index]["insturctions"];
                      Helper.wattype = watgettask[index]["type"];

                      Get.to(
                        WATParticulartaskProgressScreen(
                          instructions: Helper.WATinstrusction, 
                          category: 'WAT', type: Helper.wattype,
                        ),
                      );
              },
              child: Text(
                watgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(watgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.wattype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateWATtask(updatedTask);
