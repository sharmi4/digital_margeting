...
import 'add_ormtask_screen.dart';
import 'orm_particulartask_screen.dart';


class ORMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  ORMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<ORMStrategyScreen> createState() => _ORMStrategyScreenState();
}

class _ORMStrategyScreenState extends State<ORMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getormTask>>>>>>>${ormgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = ormgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.ormselectedstartergytitle = ormgettask[index]["taskname"];
                Helper.ormtaskId = ormgettask[index]["id"].toString();
                      Helper.ormsnumber = ormgettask[index]["snumber"];
                      Helper.ORMinstrusction = ormgettask[index]["insturctions"];
                      Helper.ormtype = ormgettask[index]["type"];

                      Get.to(
                        ORMParticulartaskProgressScreen(
                          instructions: Helper.ORMinstrusction, 
                          category: 'ORM', type: Helper.ormtype,
                        ),
                      );
              },
              child: Text(
                ormgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(ormgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.ormtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateORMtask(updatedTask);
