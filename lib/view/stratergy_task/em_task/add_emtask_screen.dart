...
import 'em_statergy_screen.dart';



class AddEMTaskScreen extends StatefulWidget {
  const AddEMTaskScreen({super.key});

  @override
  State<AddEMTaskScreen> createState() => _AddEMTaskScreenState();
}

class _AddEMTaskScreenState extends State<AddEMTaskScreen> {
  ...
  gettaskdb() async {
    emgettask = await DatabaseHelper().getEMdata();
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: emgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertEMtaskdata(addTask);
      Get.to(EMStrategyScreen(businessId: Helper.businessId,));
