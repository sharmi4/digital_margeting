...
import 'add_emtask_screen.dart';
import 'em_particulartask_screen.dart';




class EMStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  EMStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<EMStrategyScreen> createState() => _EMStrategyScreenState();
}

class _EMStrategyScreenState extends State<EMStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getemTask>>>>>>>${emgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = emgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.emselectedstartergytitle = emgettask[index]["taskname"];
                Helper.emtaskId = emgettask[index]["id"].toString();
                      Helper.emsnumber = emgettask[index]["snumber"];
                      Helper.EMinstrusction = emgettask[index]["insturctions"];
                      Helper.emtype = emgettask[index]["type"];

                      Get.to(
                        EMParticulartaskProgressScreen(
                          instructions: Helper.EMinstrusction, 
                          category: 'EM', type: Helper.emtype,
                        ),
                      );
              },
              child: Text(
                emgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(emgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.emtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateEMtask(updatedTask);
