...
import 'add_rrtask_screen.dart';
import 'rr_particulartask_screen.dart';
...
class RRStrategyScreen extends StatefulWidget {

   final String businessId; // <-- argument

  RRStrategyScreen({required this.businessId}); // <-- constructor
  @override
  State<RRStrategyScreen> createState() => _RRStrategyScreenState();
}

class _RRStrategyScreenState extends State<RRStrategyScreen> {
  ...
  gettaskdb() async {
    ...
    print(">>>>>>>>>>>>getrrTask>>>>>>>${rrgettask.length}");
  ...
  Widget _buildProgressCard() {
    int completed = rrgettask.where((task) => task["iscompleted"] == "1").length;
  ...
  Widget _buildTaskTile(int index) {
      ...
      title: Row(
        children: [
          Expanded(
            child: InkWell(
              onTap: (){
                Helper.rrselectedstartergytitle = rrgettask[index]["taskname"];
                Helper.rrtaskId = rrgettask[index]["id"].toString();
                      Helper.rrsnumber = rrgettask[index]["snumber"];
                      Helper.RRinstrusction = rrgettask[index]["insturctions"];
                      Helper.rrtype = rrgettask[index]["type"];

                      Get.to(
                        RRParticulartaskProgressScreen(
                          instructions: Helper.RRinstrusction, 
                          category: 'RR', type: Helper.rrtype,
                        ),
                      );
              },
              child: Text(
                rrgettask[index]["taskname"],
                style: primaryFont(
                  fontWeight: FontWeight.w500,
                  fontSize: 15,
    ...
    trailing: Row(
      ...
      children: [
        InkWell(
          onTap: ()async {
              final task = SeoTaskModel.fromJson(rrgettask[index]);
  ...
  builder: (context) {
    return AlertDialog(
      ...
      iscompleted: task.iscompleted, type: Helper.rrtype, businessId: Helper.businessId,
    );

    await DatabaseHelper().updateRRtask(updatedTask);
