...
import 'rr_statergy_screen.dart';





class AddRRTaskScreen extends StatefulWidget {
  const AddRRTaskScreen({super.key});

  @override
  State<AddRRTaskScreen> createState() => _AddRRTaskScreenState();
}

class _AddRRTaskScreenState extends State<AddRRTaskScreen> {
  ...
  gettaskdb() async {
    rrgettask = await DatabaseHelper().getRRdata();
    print(">>>>>>>>>>>>getpaTask>>>>>>>${rrgettask.length}");
    setState(()  {
  ...
  child: GestureDetector(
    ...
    child: Container(
      ...
      snumber: rrgettask.length+1,
      insturctions: instructionController.text,
      iscompleted:0, date: selectedDate.toString(), 
      type: "Manual", businessId: Helper.businessId,
    );
     await DatabaseHelper().insertRRtaskdata(addTask);
      Get.to(RRStrategyScreen(businessId: Helper.businessId,));
